--***********************************************************
--**               Seasons overide by Onkeen               **
--**       Based on ROBERT JOHNSON's awesome work          **
--***********************************************************
...
season.EveryTenMinutes = function()
	...
	if(currentHour ~= season.previousHour) then
		...
		if season.previousDay ~= GameTime:getInstance():getDay() then
			...
			-- [Die Tonight] Min temperature is 5 degrees at night ad max is 40 during the day
			season.min = 1;
			season.max = 60;
		...
		print("[DT-INFO] Current Hour : " .. tostring(currentHour));
		print("[DT-INFO] Current Temperature : " .. tostring(season.max - (hourLeft * tempPerHour)));
		...
		-- Temperature in the DieTonight desert is not affected by seasons
...
--			noise(goalTemp..' '..playerTemp)
			if goalTemp ~= playerTemp then
				-- you body temperature up by 10% of the gap between current and target temperature
				player:setTemperature(playerTemp + (goalTemp - playerTemp) / 10);
			end
		end
	end
end
...
-- choose a random weather whithout rain
season.updateWeather = function(month, currentHour)
	...
	if season.previousWeatherHour == 0 then
		...
		if "rain" == season.weather then
			-- The DiteTonight desert is always dry
			season.weather = "normal";
		end
...
-- Overide the game meteo system with a desertic one
